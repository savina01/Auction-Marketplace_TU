    ...
    public class StripeService : IStripeService
    {
        ...
        public async Task<bool> IsPaymentReceived(string userEmail, decimal amount)
        {
            try
            {
                ...
                var paymentIntents = await GetPaymentIntents(stripeCustomerId);

                var paymentReceived = paymentIntents.Any(intent =>
                    intent.Status == "succeeded" && intent.Amount == amount && intent.Currency == "usd");

                return paymentReceived;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"An error occurred while checking payment status: {ex.Message}");
                throw;
            }
        }
        public async Task<string> GetStripeCustomerId(string userEmail)
        {
            try
            {
            ...
            {
                Console.WriteLine($"An error occurred while retrieving Stripe customer ID: {ex.Message}");
                throw;
            }
        }

        public async Task<List<PaymentIntent>> GetPaymentIntents(string customerId)
        {
            try
            {
            ...
            {
                Console.WriteLine($"An error occurred while fetching payment intents: {ex.Message}");
                throw;
            }
        }
