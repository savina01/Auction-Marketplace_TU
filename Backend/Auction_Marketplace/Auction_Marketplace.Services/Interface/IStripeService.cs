...
namespace Auction_Marketplace.Services.Interface
{
	public interface IStripeService : IService
	{
		...
		Task<List<PaymentIntent>> GetPaymentIntents(string customerId);
		Task<string> GetStripeCustomerId(string userEmail);
		Task<bool> IsPaymentReceived(string userEmail, decimal amount);
